...
4. 결제 완료 시 이니시스가 **`/api/inicis/payment-callback`** 으로 **인증결과** 전송  
   → 서버가 이니시스 **승인 API(authUrl)** 를 호출해 실제 카드 승인(tid) 확보  
   → 성공 시 주문 `paid` 처리 후 **`/checkout/result?tid=...`** 로 리다이렉트
...
결제 금액은 **VAT 포함 1,000원**이며, 일반 요금제 선택 화면에는 노출되지 않습니다. (이니시스 최소 결제 금액 미만은 승인되지 않을 수 있음) **신용카드**로 결제한 뒤 이니시스 관리자·Supabase `orders`(paid)·Vercel `/api/inicis/payment-callback` 로그를 확인하세요.
...
- **완료 화면은 뜨는데 카드 알림·TID 없음**  
  - 예전에는 인증만 성공해도 완료 처리되던 버그가 있음. 최신 배포 후 **TID가 표시**되어야 실승인  
  - Vercel 로그에서 `[inicis payment-callback] approval` 의 `tid` 확인
